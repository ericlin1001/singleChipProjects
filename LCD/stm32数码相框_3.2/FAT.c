#include "fat.h"
#include "MMC.h"        //存储设备的扇区读写驱动，这里是SD卡
#include "string.h"
...
void FAT32_toFileName(char *dName,char *pName)
{
 ...
 for(j=0; j<i+2; j++) pName[j]=L2U(dName[j]); //将小字字符转为大写
 pName[j++]='.'; 
 i=10;
 while(dName[i--]==' ');
 k=j+i-6;
...
{
 UINT32 Cur_Clust,sec_temp,iSec,iFile,iItem;
 UINT8  flag;
 ...
 { 
  ...
  {	
   ...
   for(iFile=0; iFile<pArg->BytesPerSector; iFile+=sizeof(struct direntry))
   {
	 ...
	 //显示文件名
	 LCD_ShowString(50,330,0x07ff,0xf81f, temp_file_name);

	 //显示本文件位置
	 LCD_ShowChar(50,360,0x07ff,0xf81f,iFile/1000|0x30); 
     LCD_ShowChar(58,360,0x07ff,0xf81f,iFile%1000/100|0x30);
     LCD_ShowChar(66,360,0x07ff,0xf81f,iFile%100/10|0x30); 
     LCD_ShowChar(74,360,0x07ff,0xf81f,iFile%10|0x30); 
