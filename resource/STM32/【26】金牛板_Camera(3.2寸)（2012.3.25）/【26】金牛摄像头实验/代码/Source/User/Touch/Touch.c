...
void EXTI4_IRQHandler()
{
	...
	{
    	EXTI_ClearITPendingBit(EXTI_IMR_MR4);	 //清楚中断挂起位
...
void Save_Adjdata(void)
{
	uint8_t temp[4];
	//保存校正结果!		   							  
	temp[0]=(uint32_t)(Pen_Point.xfac*100000000)&0xff;	//保存x校正因素    
	temp[1]=(uint32_t)(Pen_Point.xfac*100000000)>>8&0xff;//保存x校正因素    
	temp[2]=(uint32_t)(Pen_Point.xfac*100000000)>>16&0xff;//保存x校正因素        
	temp[3]=(uint32_t)(Pen_Point.xfac*100000000)>>24&0xff;//保存x校正因素        
   	I2C_Write(&temp[0],SAVE_ADDR_BASE,4);

	temp[0]=(uint32_t)(Pen_Point.yfac*100000000)&0xff;//保存x校正因素    
	temp[1]=(uint32_t)(Pen_Point.yfac*100000000)>>8&0xff;//保存x校正因素    
	temp[2]=(uint32_t)(Pen_Point.yfac*100000000)>>16&0xff;//保存x校正因素        
	temp[3]=(uint32_t)(Pen_Point.yfac*100000000)>>24&0xff;//保存x校正因素 

   	I2C_Write(&temp[0],SAVE_ADDR_BASE+4,4);
	temp[0]=(uint32_t)(Pen_Point.xoff*100000000)&0xff;//保存x校正因素    
	temp[1]=(uint32_t)(Pen_Point.xoff*100000000)>>8&0xff;//保存x校正因素    
   	I2C_Write(&temp[0],SAVE_ADDR_BASE+8,2);
	//保存x偏移量
	//保存y偏移量
	temp[0]=(uint32_t)(Pen_Point.yoff*100000000)&0xff;//保存x校正因素    
	temp[1]=(uint32_t)(Pen_Point.yoff*100000000)>>8&0xff;//保存x校正因素    
