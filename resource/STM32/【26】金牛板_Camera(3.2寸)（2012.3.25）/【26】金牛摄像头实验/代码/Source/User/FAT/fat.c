...
#include "stm32f10x.h"
#include "fat.h"
#include "MMC_SD.h"        //存储设备的扇区读写驱动，这里是SD卡
#include "string.h"
...
//FAT 数据缓存区
uint8_t fat_buffer[512];//用于存储FAT数据
uint8_t LongNameBuffer[MAX_LONG_NAME_SIZE];//长文件名的缓存区
...
uint32_t FileType_Tell(uint8_t * exName)
{
    uint8_t i;
    uint8_t t;   
    for(i=0;i<20;i++)
...
{
	uint32_t bcluster; 
	uint8_t fat_base;//0~Fat_Table_Size	 
	FAT_TAB.Fat_Over=0;	   
...
unsigned long FAT_NextCluster(unsigned long cluster)
{
	...
	if(FAT32_Enable)
	{
		...
		sector=((unsigned long *)fat_buffer)[offset];//uint32_t	
	}
	else
	{
		offset=cluster%256;//计算在扇区内的偏移
		sector=((unsigned short *)fat_buffer)[offset];//uint16_t
	}			   
...
//返回值   :1,操作成功.0,操作失败
uint8_t Get_File_Info(uint32_t dir_clust,FileInfoStruct *FileInfo,uint32_t type,uint16_t *count)
{ 			  	   
	DWORD sector;
...
//buf     :数据缓存区
//返回值  :0,操作失败,1,操作成功
unsigned char F_Read(FileInfoStruct *FileInfo,uint8_t *buf)
{	
	DWORD sector;			   		  
...
//比较两个字符串相等不
//相等,返回1,不相等,返回0;
uint8_t mystrcmp(uint8_t*s1,uint8_t*s2)
{
	uint8_t len1,len2;
	len1=len2=0;
	while(*s1!='\0')
...
FileInfoStruct F_Search(uint32_t cluster,unsigned char *Name,uint32_t type)
{
	DWORD sector;			 
	DWORD tempclust;
	unsigned char cnt;
