...
typedef struct  
{
	...
	uint8_t  F_Attr;				            	//< file attr for last file accessed
	uint32_t	F_Type;
...
typedef __packed struct 
{
	...
	uint8_t  Fat_Head_Pos;                //tinyFAT表的上一个表头位置
	uint32_t Fat_Base_Head[Fat_Head_Size];//tinyFAT表的 表头数组
	uint32_t Fat_Base_Tab[Fat_Table_Size];//文件的tinyFAT基址表
	uint16_t Fat_Base_Len[Fat_Table_Size];//基址偏移量
	uint8_t  Fat_Over;                    //文件的tinyFAT表是否全部读出标记位   
}FAT_TABLE;	 
...
extern uint8_t fat_buffer[512];//FAT文件系统操作缓冲区				   
//FAT原有的函数	    
unsigned char FAT_Init(void);//初始化												  
unsigned long FAT_NextCluster(unsigned long cluster);//查找下一簇号	
uint32_t fatClustToSect(uint32_t cluster);//将簇号转换为扇区号
uint8_t Get_File_Info(uint32_t dir_clust,FileInfoStruct *FileInfo,uint32_t type,uint16_t *count);//查找文件
void F_Open(FileInfoStruct *FileInfo);//打开文件
unsigned char F_Read(FileInfoStruct *FileInfo,uint8_t *buf);//读文件，size=0代表整个文件	 
unsigned long FAT_OpenDir(BYTE * dir);//打开目录
FileInfoStruct F_Search(uint32_t cluster,unsigned char *Name,uint32_t type);//查找指定名字的文件

uint32_t FatTab_Next_Cluster(unsigned long cluster);
uint32_t FatTab_Prev_Cluster(unsigned long cluster);
