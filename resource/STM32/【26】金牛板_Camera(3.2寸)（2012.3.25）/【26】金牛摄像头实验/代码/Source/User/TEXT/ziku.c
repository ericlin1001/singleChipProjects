...
uint32_t FONT16ADDR ;//16字体存放的地址
uint32_t FONT12ADDR ;//12字体存放的地址
uint32_t UNI2GBKADDR;//UNICODE TO GBK 表存放地址	  	 
...
uint8_t Font_Init(void)
{
	uint32_t tempsys[2];//临时记录文件起始位置和文件大小
	uint8_t fontok=0;
	M45PE_Read(&fontok,24,1);//读出字库标志位,看是否存在字库
	if(fontok!=0XAA)return 1;
	M45PE_Read((uint8_t*)tempsys,0,8);//读出8个字节   
	UNI2GBKADDR=tempsys[0];					  
	//printf("tempsysgbk[0]:%d\n",tempsys[0]);
	//printf("tempsysgbk[1]:%d\n",tempsys[1]);

	M45PE_Read((uint8_t*)tempsys,8,8);//读出8个字节   
	//printf("tempsysf16[0]:%d\n",tempsys[0]);
	//printf("tempsysf16[1]:%d\n",tempsys[1]);
	FONT16ADDR=tempsys[0];
	
	M45PE_Read((uint8_t*)tempsys,16,8);//读出8个字节   
